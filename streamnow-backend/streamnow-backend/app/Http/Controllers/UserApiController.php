                    ...
                    if($action->is_verified) {
                        ...
                        if($action->status){
                            ...
                            if($action->login_by == 'manual') {
                                ...
                                $email_send = Helper::send_email($page,$subject,$action->email,$email_data)->getData();
                                ...
                                if ($email_send->success) {
                                    ...
                                    throw new Exception($email_send->error_messages);
            ...
            if ($validator->fails()) {
                ...
                if (!$model) {
                    ...
                    if ($model->save()) {
                        ...
                        $notification = NotificationTemplate::getRawContent(USER_FOLLOW, $user_model);

                        $content = $notification ? $notification : USER_FOLLOW;

                        UserNotification::save_notification($request->follower_id, $content, $request->id, USER_FOLLOW , $request->id);
                ...
                if ($user) {
                    ...
                    if ($user->user_type) {
                        ...
                        if ($call_status->success) {
                            ...
                            throw new Exception($call_status->error_messages , 101);
   ...
   /**
    ...
    public function send_notification_to_followers(Request $request) {
        ...
        foreach ($followers as $key => $value) {
            ...
            $subject = $user->name.' '.tr('new_video_streaming');

            $email_data['name'] = $value->name;

            $email_data['streamer'] = $user->name;

            $page = "emails.user.notification";
   ...
   /**
                            ...
                            if($subscription) {
                                ...
                                if($subscription->status) {
                                    ...
                                    if ($subscription->amount > $amount_convertion && $amount_convertion > 0) {
                                        ...
                                        $amount = 0;
                                        $response_array = ['success'=> true, 
                                        'data'=>['remaining_amount'=>(string) $amount,
                                            'coupon_amount'=>(string) $amount_convertion,
                                            'coupon_code'=>$model->coupon_code,
                                            'original_coupon_amount'=>(string) ($model->amount_type == PERCENTAGE ? $model->amount.'%' : Setting::get('currency').$model->amount)
                                        ]];
    ...
    public function vod_invoice(Request $request) {
        ...
        try {
            ...
            $validator = Validator::make(
                $request->all(), array(
                    'video_id'=>'required|exists:vod_videos,unique_id,status, '.VOD_APPROVED_BY_USER.',admin_status,'.VOD_APPROVED_BY_ADMIN.',publish_status,'.VIDEO_PUBLISHED,',is_pay_per_view,'.PPV_ENABLED,
            ));
            ...
            if ($validator->fails()) {
               ...
               'title', 'description', 'amount', 
                ...
                DB::raw("'$currency' as currency"))
                ->where('unique_id',$request->video_id)->first();
    ...
    public function custom_live_videos_delete(Request $request) {
        ...
        if ($model->delete()) {
            ...
            if($image) {
                ...
                Helper::delete_avatar( "uploads/live_videos", $model->image);
