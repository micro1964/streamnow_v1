		            ...
		            <div class="tab-content">
		              <div class="active tab-pane" id="activity">
				            ...
				            <table class="table table-striped">
				            	...
				            	<tr>
				            		<th>{{tr('login_type')}}</th>
				            		<td>{{$data->login_by ? $data->login_by : "-"}}</td>
				            	</tr>
				            	...
				            	<tr>
				            		...
				            		<td>{{$data->device_type ? $data->device_type : "-"}}</td>
				            	</tr>

				            	<tr>
				            		<th>{{tr('register_type')}}</th>
				            		<td>{{$data->register_type ? $data->register_type : "-"}}</td>
				            	</tr>
				            	...
				            	<tr>
				            		<th>{{tr('no_of_days')}}</th>
				            		<td>{{$data->no_of_days ? $data->no_of_days : "0"}}</td>
				            	</tr>

				            	<tr>
				            		<th>{{tr('expiry_date')}}</th>
				            		<td>{{$data->no_of_days > 0 ? common_date($data->expiry_date,Auth::guard('admin')->user()->timezone) : '-'}}</td>
				            	</tr>
				            	...
				            	<tr>
									...
									<td>
										...
										@else
											...
											<a href="{{route('admin.become.creator' , ['id'=>$data->id])}}" onclick="return confirm('Are you sure ?')">		
		           		...
		           		<table class="table table-bordered table-striped datatable-withoutpagination">
							...
							<tbody>
								...
								@foreach($followers as $i => $follower)
								    ...
								    <tr>
								      	<td>{{$i+1}}</td>
								      	<td><a href="{{route('admin.users.view', $follower->follower_id)}}" target="_blank">{{$follower->name}}</a></td>
		           		...
		           		<table class="table table-bordered table-striped datatable-withoutpagination">
							...
							<tbody>
								...
								@foreach($followings as $i => $following)
								    ...
								    <tr>
								      	<td>{{$i+1}}</td>
								      	<td><a href="{{route('admin.users.view', $following->follower_id)}}" target="_blank">{{$following->name}}</a></td>
		           		...
		           		<table class="table table-bordered table-striped datatable-withoutpagination">
							...
							<tbody>
								...
								@foreach($blockers as $i => $blocker)
								    ...
								    <tr>
								      	<td>{{$i+1}}</td>
								      	<td><a href="{{route('admin.users.view', $blocker->block_user_id)}}" target="_blank">{{$blocker->name}}</a></td>
								      	<td>{{$blocker->email}}</td>
								    </tr>					
							...
							<tbody>
								...
								@foreach($videos as $i => $video)
								    ...
								    <tr>
								      	...
								      	<td><a href="{{route('admin.videos.view' , $video->id)}}" target="_blank">{{$video->title}}</a></td>
		           		...
		           		<table class="table table-bordered table-striped datatable-withoutpagination">
							...
							<tbody>
								...
								@foreach($vod_videos as $i => $vod_video)
								    ...
								    <tr>
								      	<td>{{$i+1}}</td>
								      		<td><a href="{{route('admin.vod-videos.view',$vod_video->vod_id)}}">{{$vod_video->title}}</a></td>
