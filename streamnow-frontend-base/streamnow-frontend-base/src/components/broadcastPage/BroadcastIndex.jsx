...
connection.socketURL = "https://appswamy.com:9001/";
...
class BroadcastIndex extends BroadcastHelper {
  ...
  stopBroadcastAPI = () => {
    ...
    api.postMethod("live_videos_broadcast_stop", inputData).then((response) => {
      if (response.data.success) {
        ToastContent(
          this.props.toastManager,
          response.data.message + "Starting new streaming...",
          "success"
        );
      } else {
        ToastContent(this.props.toastManager, response.data.error, "error");
